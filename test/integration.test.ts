...
describe("TuneChain — Integration Test (Skeleton Tuần 1)", function () {
    ...
    // ── Nhóm 5: Hàm uploadTrack() — skeleton revert ─────────────
    describe("TC-INT-05 — uploadTrack() revert đúng (skeleton tuần 1)", function () {
        it("uploadTrack() revert với message 'not implemented yet'", async function () {
            const { tuneChain, deployer } = await deployContracts();

            // Skeleton tuần 1: tất cả hàm revert với message cố định
            await expect(
                tuneChain.connect(deployer).uploadTrack(
                    "QmTestCID123456789",  // ipfsHash giả
                    "Bài Test"             // title giả
                )
    ...
    // ── Nhóm 6: Hàm tipTrack() — skeleton revert ────────────────
    describe("TC-INT-06 — tipTrack() revert đúng (skeleton tuần 1)", function () {
        it("tipTrack() revert với message 'not implemented yet'", async function () {
            const { tuneChain, listener } = await deployContracts();

            // trackId = 0, amount = 10 TCT (18 decimals)
            await expect(
                tuneChain.connect(listener).tipTrack(0n, ethers.parseEther("10"))
            ).to.be.revertedWith(NOT_IMPLEMENTED);
        });
    });

    // ── Nhóm 7: Hàm withdrawTips() — skeleton revert ────────────
    describe("TC-INT-07 — withdrawTips() revert đúng (skeleton tuần 1)", function () {
        it("withdrawTips() revert với message 'not implemented yet'", async function () {
            const { tuneChain, deployer } = await deployContracts();

            await expect(
                tuneChain.connect(deployer).withdrawTips()
            ).to.be.revertedWith(NOT_IMPLEMENTED);
        });
    });
